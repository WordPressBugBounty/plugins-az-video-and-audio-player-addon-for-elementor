...
use LeanPL\Admin_New\All_Playlists_Data;

$data = new All_Playlists_Data();
...
$columns = [
  ...
  'items'     => ['label' => __('TYPE', 'vapfem'),      'width' => 'lpl-w-[150px] lpl-shrink-0 lpl-ml-auto'],
...
?>
<div class="lpl-admin lpl-all-playlists-new-page">
  <?php
  ...
  <div
    ...
    <div
      ...
      <div
        ...
        <div
          ...
          <?php esc_html_e('Playlists', 'vapfem'); ?>
        </div>
        <div
          class="lpl-text-[13px]/[normal] lpl-box-border lpl-text-[#6A7180] lpl-font-normal lpl-text-left [white-space:nowrap]">
          <?php echo esc_html(sprintf(
            /* translators: 1: total playlists, 2: video count, 3: audio count. */
            __('%1$d playlists · %2$d video · %3$d audio', 'vapfem'),
            $counts['all'],
            $counts['video'],
            $counts['audio']
          )); ?>
      ...
      <a href="<?php echo esc_url(admin_url('admin.php?page=lean-playlist-edit')); ?>"
        data-lpl-add-playlist-trigger
        ...
        <div class="lpl-box-border lpl-text-left [white-space:nowrap]">
          <?php esc_html_e('Add Playlist', 'vapfem'); ?>
        </div>
      </a>
    </div>
    <div
      class="lpl-box-border lpl-w-full lpl-h-fit [box-shadow:0px_10px_30px_0px_#1B22330F] lpl-flex lpl-flex-col lpl-gap-0 lpl-justify-start lpl-items-start lpl-bg-[#FFFFFF] [outline:1px_solid_#E3E6EB] [outline-offset:-0.5px] lpl-rounded-[16px] lpl-overflow-hidden">
    <?php
    $page_slug = 'lean-player-playlist';
    $search_placeholder = __('Search playlists', 'vapfem');
    $filter_groups = [
      ['heading' => __('TYPE', 'vapfem'), 'filter' => 'type', 'options' => [
    ...
    $toolbar_row_class = 'lpl-relative lpl-box-border lpl-w-full lpl-h-fit lpl-shrink-0 lpl-flex lpl-flex-row lpl-gap-[16px] lpl-p-[16px_20px] lpl-justify-between lpl-items-center [border-width:0px_0px_1px_0px] [border-style:solid] [border-color:#EEF0F3] [margin:0px_0px_-0.5px_0px]';
    $toolbar_left_class = 'lpl-box-border [flex:1_1_0] lpl-min-w-0 lpl-h-fit lpl-flex lpl-flex-row lpl-gap-[6px] lpl-justify-start lpl-items-center';
    $toolbar_right_class = 'lpl-box-border [flex:1_1_0] lpl-min-w-0 lpl-h-fit lpl-flex lpl-flex-row lpl-gap-[10px] lpl-justify-end lpl-items-center';
      ...
      <div
        ...
        <div
          ...
          <div data-lpl-checkbox data-lpl-select-all role="checkbox" tabindex="0" aria-checked="false"
            ...
            <?php echo \LeanPL\Admin_New\Admin_New_Icons::get( 'checkbox-check' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
          </div>
        </div>
      ...
      if (empty($rows)):
        ...
        <div
          ...
          <div class="lpl-text-[15px]/[normal] lpl-box-border lpl-text-[#6A7180] lpl-font-semibold lpl-text-center">
            <?php esc_html_e('No playlists found', 'vapfem'); ?>
          </div>
          <a href="<?php echo esc_url(admin_url('admin.php?page=lean-playlist-edit')); ?>"
            data-lpl-add-playlist-trigger
        ...
        foreach ($rows as $row):
          include LEANPL_DIR . '/includes/admin-new/partials/list/playlist-row.php';
        endforeach;
      endif;
      ?>
