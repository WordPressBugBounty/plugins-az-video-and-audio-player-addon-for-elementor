...
/**
 * One row of the "All Players (New)" table.
 *
 * Expected $row shape (see All_Players_Data::build_row):
 *   id, title, edit_url, status, status_label, type, type_label,
 *   format_label, source_key, source_label, thumb_url, created,
 *   created_ts, shortcode.
 *
...
<div
  data-lpl-row
  data-lpl-row-edit-url="<?php echo esc_url( $row['edit_url'] ); ?>"
  data-lpl-id="<?php echo esc_attr( (string) $row['id'] ); ?>"
  data-lpl-type="<?php echo esc_attr( $row['type'] ); ?>"
  data-lpl-source="<?php echo esc_attr( $row['source_key'] ); ?>"
  ...
  class="lpl-box-border lpl-w-full lpl-min-h-[67.5px] lpl-shrink-0 lpl-flex lpl-flex-row lpl-gap-0 lpl-p-[14px_20px] lpl-justify-start lpl-items-center [border-width:0px_0px_1px_0px] [border-style:solid] [border-color:#EEF0F3] [margin:0px_0px_-0.5px_0px] lpl-transition-colors hover:lpl-bg-[#F9FAFB]"
>
  <div
  ...
  >
    <div
      class="lpl-box-border lpl-w-[60px] lpl-shrink-0 lpl-h-[40px] lpl-flex lpl-flex-row lpl-gap-0 lpl-justify-center lpl-items-center lpl-overflow-hidden <?php echo $row['type'] === 'audio' ? 'lpl-bg-[#EEEEFE]' : 'lpl-bg-[#1E2634]'; ?> lpl-rounded-[9px]"
    >
      <?php if ( ! empty( $row['thumb_url'] ) ) : ?>
        <img
          ...
          alt=""
          loading="lazy"
          class="lpl-object-cover lpl-w-full lpl-h-full lpl-rounded-[9px]"
        />
    ...
    >
      ...
      >
        <?php echo esc_html( $row['type_label'] . ' · ' . $row['format_label'] ); ?>
      </div>
    </div>
  </div>
  <div
    class="lpl-box-border <?php echo esc_attr( $columns['source']['width'] ); ?> lpl-h-fit lpl-flex lpl-flex-row lpl-gap-[8px] lpl-justify-start lpl-items-center"
  >
    <div
      class="lpl-box-border lpl-w-[32px] lpl-shrink-0 lpl-h-[32px] lpl-flex lpl-flex-row lpl-gap-0 lpl-justify-center lpl-items-center lpl-bg-[#F3F4F7] lpl-rounded-[9px]"
    >
      <?php if ( $row['type'] === 'audio' ) : ?>
        <?php echo \LeanPL\Admin_New\Admin_New_Icons::get( 'source-audio' ); ?>
      <?php else : ?>
        <?php echo \LeanPL\Admin_New\Admin_New_Icons::get( 'source-video' ); ?>
      <?php endif; ?>
    </div>
    <div
